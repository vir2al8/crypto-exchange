...
fun validationOperationCorrect(command: CommonCommand, processor: OrderProcessor) = runBlocking {
...
fun validationOperationNone(command: CommonCommand, processor: OrderProcessor) = runBlocking {
    val ctx = CommonContext(
        command = command,
        state = CommonState.NONE,
        workMode = CommonWorkMode.TEST,
        orderRequest = CommonOrder(
            ...
            walletId = CommonWalletId("123-234-abc-ABC"),
            type = CommonOrderType.LIMIT,
            operation = CommonOrderOperation.NONE,
        ),
    )
    ...
    assertEquals("operation", error?.field)
    assertContains(error?.message ?: "", "operation")
}